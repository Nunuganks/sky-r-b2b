...
const CompactImageGallery: React.FC<CompactImageGalleryProps> = ({ value = [], onChange }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium">Медии</h3>
          <p className="text-sm text-gray-600">Upload product images. First image will be used as the main product image.</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {images.map((image: any, index: number) => (
          ...
          >
            <div className="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-200 bg-white">
              ...
              {/* Main Image Label */}
              {image.isMain && (
                <div className="absolute top-1 right-1 bg-gray-600 text-white text-xs px-2 py-1 rounded">
                  Основен
                </div>
              )}
              ...
              {/* Selection Checkbox */}
              {selectedImages.includes(image.id || index.toString()) && (
                <div className="absolute top-1 left-1 bg-purple-500 text-white rounded-full w-5 h-5 flex items-center justify-center">
        ...
        <div className="w-20 h-20 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-purple-400 transition-colors bg-white">
...
export default CompactImageGallery; 